...
\begin{exercise} 3-4-2-2 (\textbf{RNN})
    ...
    \subsubsection*{4.2}
    We will show that
    $\norm{\pder{\mathbf{g}_{T}}{\mathbf{g}_{1}}} \xrightarrow{T\to \infty}0$.
    Firstly, note that $\pder{g_{n}}{g_{n-1}} = W \sigma'(g_{n-1})$. Therefore,
    \begin{align*}
      \pder{g_{n}}{g_{1}} &= \prod_{k = 2}^{n} \pder{g_{k}}{g_{k-1}}\\
      &= \prod_{k=2}^{n} W \sigma'(g_{k-1}).
    \end{align*}
    If we take the norm and use the $L_{2}$ properties, we have:
    \begin{align*}
      \norm{\pder{g_{n}}{g_{1}}} &= \norm{\prod_{k=2}^{n} W \sigma'(g_{k-1})}\\
    ...
    \end{align*}
    If we considered $\norm{\pder{g_{n}}{h_{0}}}$ in place of
    $\norm{\pder{g_{n}}{g_{1}}}$, we would obtain the same result
    (i.e. $\norm{\pder{g_{n}}{h_{0}}} \xrightarrow{n \to \infty}0$) by noting that
    $\pder{g_{n}}{h_{0}} =\pder{g_{n}}{g_{1}} \pder{g_{1}}{h_{0}}$ and that
    $\pder{g_{1}}{h_{0}} = W$.
    \subsubsection*{4.3}
